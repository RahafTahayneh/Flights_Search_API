...
class Flights:
    ...
    def check_and_list_one_way(self):
        ...
        if City.objects.filter(city_name__in=flying_list).exists():
            origin_airport = Airport.objects.filter(city_id=City.objects.get(city_name=self.flying_from)
                                                    .city_id).values('airport_id')
            origin_airport_list = []
            for airport in origin_airport:
                origin_airport_list.append(airport['airport_id'])

            arrival_airport = Airport.objects.filter(city_id=City.objects.get(city_name=self.flying_to)
                                                     .city_id).values('airport_id')
    ...
    def check_and_list_roundtrip(self):
        ...
        try:
            ...
            if City.objects.filter(city_name=self.flying_from).exists():
                ...
                list_of_journeys = Journey.objects.filter(
                        origin_airport__in=origin_airport_list, arrival_airport=
                        Airport.objects.get(airport_name=self.flying_to),
                        Departure_date_time__startswith=self.departure_date.date(),
                        journey_type='Roundtrip').values()
                return list(list_of_journeys)
                ...
                list_of_journeys = Journey.objects.filter(
                    origin_airport=Airport.objects.get(airport_name=self.flying_from), arrival_airport__in=
                    arrival_airport_list, Departure_date_time__startswith=self.departure_date.date(),
                    journey_type='Roundtrip').values()
                return list(list_of_journeys)
